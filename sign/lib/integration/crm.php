...
class CRM
{
	...
	private static function prepareUfFieldValue($value, string $entityType, string $fieldCode)
	{
		...
		if ($entities[$entityType][$fieldCode] ?? null)
		{
			...
			if ($typeId === 'boolean')
			{
				$value = array_map(function($itemVal)
				{
			...
			else if ($typeId === 'datetime' && $value instanceof \Bitrix\Main\Type\DateTime)
			{
				$value = (string)$value;
			}
		}
	 ...
	 */
	public static function getEntityFieldValue(int $entityId, string $fieldCode): ?array
	{
		...
		if (strpos($entityFieldCode, 'UF_CRM_') === 0)
		{
	...
	public static function getMyDefaultPresetId(int $documentEntityId, int $companyId = 0): ?int
	{
		if (!\Bitrix\Main\Loader::includeModule('crm'))
		{
			return null;
	...
	public static function createDefaultRequisite(int $documentEntityId, int $entityId, int $entityType): AddResult|Result
	{
		...
		$saveRequisiteResult = EntityRequisite::getSingleInstance()->add($requisiteFields);
