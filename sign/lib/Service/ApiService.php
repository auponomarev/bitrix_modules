...
class ApiService
{
	...
	public function get(string $endpoint): Main\Result
	{
	...
	): Main\Result
	{
		...
		$data = Main\Text\Encoding::convertEncoding($data, SITE_CHARSET, 'UTF-8');
		$data = !empty($data) ? Main\Web\Json::encode($data) : "";
		$http->query($method, $url, $data);

		$data = $http->getResult();
		$data = Main\Text\Encoding::convertEncoding($data, 'UTF-8', SITE_CHARSET);
	...
	{
		$message = (new ErrorHandler())->getDefaultError();
